...
import { getDailyPnl, getTeamRecords, getWallet, type DailyPnl, type TeamRecord } from '@/lib/supabase'
...
function periodToDays(p: Period): number {
  const map: Record<Period, number> = {
    today: 1, '7d': 7, '30d': 30, '90d': 90, '1y': 365, all: 9999,
  }
  return map[p]
}
...
function PnlTooltip({ active, payload, label }: any) {
  ...
  return (
    <div className="bg-surface border border-border rounded-xl px-4 py-3 shadow-card-md text-sm">
      ...
      <p className={`font-semibold tabular font-mono ${val >= 0 ? 'text-profit' : 'text-loss'}`}>
        {val >= 0 ? '+' : ''}€{fmt(val)}
      </p>
    </div>
  )
...
export default function DashboardPage() {
  ...
  const cumulative = filteredDaily.reduce<{ date: string; cumPnl: number; dailyPnl: number }[]>(
    (acc, row) => {
  ...
  const totalBets  = filteredDaily.reduce((s, r) => s + Number(r.bets_placed), 0)
  ...
  const pieData = [
    ...
    { name: 'Draws',  value: totalBets - totalWins - totalLoss, color: '#F59E0B' },
  ].filter(d => d.value > 0)
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-10 space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-display-md font-display text-primary">Dashboard</h1>
          <p className="text-muted text-sm mt-1">Model performance and betting trends</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          <div key={label} className="card px-5 py-4">
            ...
            <p className={`text-xl font-display font-semibold tabular ${color}`}>{value}</p>
      ...
      <div className="card px-6 py-5">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h3 className="font-display font-semibold text-primary">Cumulative P&amp;L</h3>
            <p className="text-xs text-muted mt-0.5">Running total since period start</p>
          </div>
          <span className={`text-sm font-semibold tabular font-mono ${
      ...
      <div className="grid md:grid-cols-3 gap-4">
        ...
        <div className="card px-6 py-5 md:col-span-2">
          <h3 className="font-display font-semibold text-primary mb-1">Daily P&amp;L</h3>
          <p className="text-xs text-muted mb-5">Per-day profit and loss</p>
          {loading ? (
            <div className="h-44 skeleton rounded-xl" />
          ) : (
            <ResponsiveContainer width="100%" height={176}>
              <BarChart data={cumulative} margin={{ top: 4, right: 4, left: 0, bottom: 0 }} barCategoryGap="30%">
                <CartesianGrid stroke="#E2E8F0" strokeDasharray="3 3" vertical={false} />
                <XAxis
        ...
        <div className="card px-6 py-5">
          <h3 className="font-display font-semibold text-primary mb-1">Outcomes</h3>
              ...
              <div className="flex gap-4 text-xs">
                {pieData.map(d => (
                  <div key={d.name} className="flex items-center gap-1.5">
                    ...
                    <span className="font-semibold tabular text-primary">{d.value}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      <div className="card overflow-hidden">
        <div className="px-6 py-5 border-b border-border flex items-center justify-between">
          <div>
            <h3 className="font-display font-semibold text-primary">Performance by club</h3>
            <p className="text-xs text-muted mt-0.5">P&amp;L from bets involving each team</p>
          </div>
          <select
            value={clubFilter}
          ...
          >
            ...
            {teams.map(t => (
              <option key={t.team_id} value={t.tla}>{t.team_name}</option>
            ))}
          </select>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-subtle/30">
                {['Club', 'Bets', 'Wins', 'Losses', 'Win rate', 'P&L'].map(h => (
            ...
            <tbody>
              {(clubFilter === 'all' ? teams : teams.filter(t => t.tla === clubFilter))
                .map(team => (
                <tr key={team.team_id} className="border-b border-border/50 table-row-hover last:border-0">
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-2.5">
                      <div className="w-6 h-6 rounded-full bg-subtle border border-border
                                      flex items-center justify-center text-[9px] font-bold text-muted">
                        {team.tla}
                      </div>
                      <span className="font-medium text-primary">{team.team_name}</span>
                    </div>
                  </td>
                  <td className="px-5 py-3.5 tabular font-mono text-xs">{team.total_bets ?? 0}</td>
                  <td className="px-5 py-3.5 tabular font-mono text-xs text-profit">{team.wins ?? 0}</td>
                  <td className="px-5 py-3.5 tabular font-mono text-xs text-loss">{team.losses ?? 0}</td>
                  <td className="px-5 py-3.5">
                  ...
                  <td className="px-5 py-3.5">
                    <span className={`tabular font-mono text-xs font-semibold ${
                      ...
                      {(team.total_pnl ?? 0) >= 0 ? '+' : ''}€{fmt(team.total_pnl ?? 0)}
                    </span>
                  </td>
                </tr>
              ))}
              {teams.length === 0 && !loading && (
                <tr>
                  <td colSpan={6} className="px-5 py-10 text-center text-muted text-sm">
                    No betting data yet. Results will appear after the first matchday.
                  </td>
                </tr>
              )}
            </tbody>
