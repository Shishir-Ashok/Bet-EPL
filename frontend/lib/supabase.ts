...
export async function getDailyPnl(days: number = 90): Promise<DailyPnl[]> {
  ...
  const { data } = await supabase
    .from('daily_pnl')
    .select('*')
    .gte('bet_date', since.toISOString().split('T')[0])
  ...
  const { data } = await supabase
    ...
    .select(`
      *,
      matches (
        kickoff_time, season,
        home:teams!matches_home_team_id_fkey(name, short_name, tla),
        away:teams!matches_away_team_id_fkey(name, short_name, tla)
      )
    `)
