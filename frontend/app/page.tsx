...
function actionLabel(action: string | null) {
  ...
  const map: Record<string, { label: string; color: string }> = {
    ...
    BET_DRAW: {
      label: "Bet Draw",
      color: "text-draw    bg-draw-bg       border-draw-border",
    },
    BET_AWAY: {
      label: "Bet Away",
      color: "text-profit  bg-profit-bg     border-profit-border",
    },
    PASS: {
      label: "Pass",
      color: "text-muted   bg-subtle        border-border",
    },
  };
      ...
      <section className="space-y-10">
        ...
        <div className="space-y-4 animate-fade-up">
          ...
          <div className="flex items-baseline gap-4">
            <h1
              className={`text-display-2xl font-display tabular leading-none ${
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 animate-fade-up animate-delay-100 opacity-0-init">
          {[
            ...
            <div key={label} className="card px-5 py-4 space-y-1">
              ...
              <p className="text-xl font-display font-semibold tabular text-primary">
                {value}
              </p>
                ...
                >
                  ...
                  {action && (
                    <div className="flex-shrink-0">
                      <span
                        className={`text-xs font-semibold px-2.5 py-1 rounded-lg border ${action.color}`}
                      >
                        {action.label}
                      </span>
                    </div>
