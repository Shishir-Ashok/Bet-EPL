...
function outcomeConfig(outcome: string | null) {
  if (outcome === 'WIN')  return { label: 'Win',  badge: 'badge-profit', row: 'bg-profit-bg/30 hover:bg-profit-bg/50' }
  if (outcome === 'LOSS') return { label: 'Loss', badge: 'badge-loss',   row: 'bg-loss-bg/20  hover:bg-loss-bg/40'  }
...
export default function HistoryPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-10 space-y-8">
      ...
      {/* Header */}
      <div>
        <h1 className="text-display-md font-display text-primary">Bet History</h1>
        <p className="text-muted text-sm mt-1">Complete log of every virtual bet placed by the model</p>
      </div>

      {/* Summary row */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'Total bets',   value: bets.length.toString(),              color: 'text-primary' },
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-subtle/40">
                {['Date', 'Match', 'Season', 'Bet on', 'Odds', 'Stake', 'Outcome', 'P&L', 'Balance after'].map(h => (
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-5 py-3.5 whitespace-nowrap">
                      <span className="text-xs tabular text-muted">{kickoff}</span>
                    </td>
                    ...
                    <td className="px-5 py-3.5">
                      <span className="text-xs text-muted">{m?.season ?? '—'}</span>
                    </td>
                    ...
                    {/* Odds */}
                    <td className="px-5 py-3.5">
                      <span className="tabular font-mono text-xs text-primary">{bet.odds.toFixed(2)}</span>
                    </td>

                    {/* Stake */}
                    <td className="px-5 py-3.5">
                      <span className="tabular font-mono text-xs text-primary">€{fmt(bet.stake)}</span>
                    </td>
                    ...
                    <td className="px-5 py-3.5">
                      <span className="tabular font-mono text-xs text-muted">
                        {bet.balance_after != null ? `€${fmt(bet.balance_after)}` : '—'}
                      </span>
                    </td>
                  </tr>
                )
